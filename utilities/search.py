    ...
    def select_fields(self):
        if self.query.fields:
            for field in self.fields:
                if field.name in self.query.fields:
                    field.include = True
                else:
                    field.include = False
...
class Query:
    ...
    def extract_field_names(self):
        ...
        for term in temp:
            if ':' in term:
                self.field_term.append(term.split(':'))
            else:
                self.fields.append(term.lower())

    def extract_special_terms(self):
        self.special_terms = [w[1:].lower() for w in self.words if len(w) > 1 and w[0] == '*']
        if 'complete' in self.special_terms:
            self.completeness = True
        elif 'incomplete' in self.special_terms:
            self.completeness = False
        else:
            self.completeness = None
        if 'approved' in self.special_terms:
            self.approval = True
        elif 'unapproved' in self.special_terms:
            self.approval = False
        else:
            self.approval = None
