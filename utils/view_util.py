...
class Event:
    ...
    def set_changes(self):
        try:
            self.cf_dict = eval(self.event.changed_fields)
        except:
            raise ValueError('could not create dict from:',
    ...
    def set_time(self):
        ...
        if self.minutes_ago < 3:
            ts = 'a moment ago'
        elif self.changed_recent:
            ts = str(self.minutes_ago) + ' minutes ago'
        elif self.hours_ago < 6:
            ts = str(self.hours_ago) + ' hours ago'
        elif self.changed_today:
            ts = dt.strftime('today %H:%M')
        elif self.days_ago < 2:
            ts = dt.strftime('yesterday %H:%M')
        elif self.days_ago < 6:
            ts = dt.strftime('last %A %H:%M')
        elif self.changed_thisyear:
            ts = dt.strftime('%d %B')
        else:
            ts = self.time_str_day_short
        self.time_str = ts
    ...
    def model(self):
        if not hasattr(self, '_model'):
            try:
                self._model = apps.get_model(self.app_name, self.model_name)
            except:
                self._model = False
        return self._model
    ...
    def object_str(self):
        try:
            return self.model.objects.get(pk=self.model_pk).__str__()
        except:
    ...
    def link(self):
        ...
        if 'relation' in self.model_name:
            model = self.model
            try:
                instance = model.objects.get(pk=self.model_pk)
            except:
                return ''
            if not hasattr(instance, 'primary'): return ''
            print(model, instance.primary)
            app_name, model_name = instance2names(instance.primary)
            return "/" + app_name + "/" + model_name.lower() + "/new/" + str(instance.primary.pk)
        return "/" + self.app_name + "/" + self.model_name.lower() + "/new/" + str(self.model_pk)
...
class Change:
    def __init__(self, user, time, field_name, state, related_name, related_instance=None):
        ...
        if not related_instance or not related_name:
            self.new_state = state[1]
        else:
            self.new_state = state[1] + ' (' + related_instance.__str__() + ')'
...
def make_tabs(tab_type, focus=0, focus_names=''):
    ...
    if tab_type == 'church':
        t = 'Saints, Objects, Inscriptions, Liturgical manuscripts, External links'
        relations = Tab(t, focus)
        return Tabs([minimize, relations], 'minimize,relations', focus_names)
    if tab_type == 'object':
        t = 'Saints, Churches'
